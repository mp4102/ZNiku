...
export function ChapterOverlapPreview({ preview }: { readonly preview: OverlapFullEnvelope | SourceAlignedFullEnvelope | SourceAdmittedFullEnvelope }) {
  ...
  return <section className="creator-step creator-confirm" aria-label="确认重叠补帧工作流">
    ...
    <section className="creator-media-summary" aria-label="重叠流程素材摘要"><h4>素材与处理</h4><dl>
      ...
      {sourceAligned && <div><dt>第 0 步外部修复</dt><dd>{mr?.mode === 'external' ? `${mr.model_name} · ${mr.model_version ?? '版本未声明'} · ${(mr.declared_container ?? 'mp4').toUpperCase()} · 保留原片时间轴` : '关闭，直接使用原片'}</dd></div>}
    ...
    <section className="creator-workflow-summary"><h4>处理顺序</h4><p>{mr?.mode === 'external' ? '外部马赛克修复 → ' : ''}分章与分叶 → 外部逐叶增强 → 章内合并 → 收集相邻章节增强上下文 → 外部 Aion 补帧 → 精确裁边 → 连续编码与原音轨封装。</p><p>上下文步骤需要所引用的相邻章节先完成增强。等待没有估算倒计时；外部原始结果会保留，裁边结果另存。</p>{sourceAligned && <p>确认后才把以上处理设置应用为普通节点图；现有素材分析和 Run snapshot 不会被修改。外部修复未提交时，不启动分章。</p>}</section>
