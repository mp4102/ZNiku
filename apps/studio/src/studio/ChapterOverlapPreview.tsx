...
import type { PreparedSourceFullEnvelope } from './prepared-source-contracts'
import type { ColorPreparedSourceFullEnvelope } from './prepared-color-contracts'
import type { WorkFullEnvelope } from './working-source-contracts'

export function ChapterOverlapPreview({ preview }: { readonly preview: OverlapFullEnvelope | SourceAlignedFullEnvelope | PreparedSourceFullEnvelope | ColorPreparedSourceFullEnvelope | WorkFullEnvelope }) {
  const working = preview.contract_version === '0.3.4-work.1'
  const colorPrepared = preview.contract_version === '0.3.4-color.1'
  const prepared = preview.contract_version === '0.3.4' || colorPrepared || working
  const sourceAligned = preview.contract_version === '0.3.3' || prepared
  ...
  return <section className="creator-step creator-confirm" aria-label="确认重叠补帧工作流">
    ...
    <div className="creator-profile-result" role="status"><strong>{working ? 'ZNIKU 普通工作源与重叠 FI · 0.3.4-work.1 · 待真实验收' : colorPrepared ? 'ZNIKU 工作色彩解释与重叠 FI · 0.3.4-color.1 · 待真实验收' : prepared ? 'ZNIKU 工作源准备与重叠 FI · 0.3.4 · 待真实验收' : sourceAligned ? 'ZNIKU 原片规划与重叠 FI · 0.3.3 · 待真实验收' : 'ZNIKU 重叠 FI 候选 · 待真实验收'}</strong><span>{preview.node_count} 个节点 · {preview.edge_count} 条连线 · {preview.plan.chapter_count} 章 · {preview.plan.leaf_count} 个处理段</span></div>
    {prepared && <p>{working ? '本计划使用已检查工作参考的帧数与时间轴，所选音频独立绑定。新参考重新规划，不继承旧结果；准备副本不是马赛克修复产物。' : '本计划使用已准入工作参考的时间轴。原件、工作参考、有效处理视频与音频分别绑定；准备副本不是 MR 产物，不继承旧工程结果。'}</p>}
    ...
    <section className="creator-media-summary" aria-label="重叠流程素材摘要"><h4>素材与处理</h4><dl>
      <div><dt>{prepared ? '工作参考帧数 / 帧率' : '源帧数 / 帧率'}</dt><dd>{preview.plan.source.frame_count} 帧 · {preview.plan.source.frame_rate} fps</dd></div>
      <div><dt>每段最长时长</dt><dd>{preview.plan.settings.leaf_max_minutes} 分钟 · 不跨章节</dd></div>
      {sourceAligned && <div><dt>第 0 步外部马赛克修复</dt><dd>{mr?.mode === 'external' ? `${mr.model_name} · ${mr.model_version ?? '版本未声明'} · ${(mr.declared_container ?? 'mp4').toUpperCase()} · 保留${prepared ? '工作参考' : '原片'}时间轴` : working ? '关闭，直接使用已检查的工作参考' : prepared ? '关闭，直接使用已准入工作参考' : '关闭，直接使用原片'}</dd></div>}
    ...
    <section className="creator-workflow-summary"><h4>处理顺序</h4><p>{mr?.mode === 'external' ? '外部马赛克修复 → ' : ''}分章与分叶 → 外部逐叶增强 → 章内合并 → 收集相邻章节增强上下文 → 外部 Aion 补帧 → 精确裁边 → 连续编码与{working ? '工作源绑定音轨' : '原音轨'}封装。</p><p>上下文步骤需要所引用的相邻章节先完成增强。等待没有估算倒计时；外部原始结果会保留，裁边结果另存。</p>{sourceAligned && <p>确认后才把以上处理设置应用为普通节点图；现有素材分析和 Run snapshot 不会被修改。外部修复未提交时，不启动分章。</p>}</section>
