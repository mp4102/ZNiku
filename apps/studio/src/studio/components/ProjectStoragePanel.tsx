...
function failureMessage(error: unknown): string {
  ...
  if (code === 'E_PROJECT_STORAGE_INDEX_TARGET') return '文件目录.html 位置已被其他文件占用，未覆盖原文件。请检查该文件并自行保留或改名后再生成。'
    ...
    <div className="retry-impact project-storage-panel" aria-busy={isBusy}>
      ...
      {inspection && <>
        <dl className="handoff-import-review">
          ...
          <div><dt>目录组织</dt><dd>{inspection.storage.layout === 'readable' ? '可读目录：章节／工序、任务名与执行批次' : 'UUID 旧布局：保留原有绑定，不会自动整理'}</dd></div>
        ...
        <section aria-label="可读目录与文件索引"><h3>找文件与整理目录</h3>
          <p>可读目录把章节、工序、任务名和执行批次放在一起；同名任务和每次重试仍各自隔离。整理不会重跑媒体、改动节点参数或自动清理历史。</p>
          ...
          <p>整理会复制到所选父文件夹下新的同名 .data 目录；逐文件比对后才切换引用，原目录全部保留。不会直接重命名正在使用的 UUID 目录。</p>
        ...
        <section aria-label="更改工程数据位置"><h3>仅更改位置，保留目录结构</h3>
          <p>只想换盘而不整理目录时使用这里；如需把 UUID 目录改为可读名称，请使用上方“预览整理”。</p>
