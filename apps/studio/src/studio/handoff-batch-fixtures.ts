...
export const batchObservation: HandoffBatchObserveEnvelope = { ...batchBinding, inbox_path: 'D:/synthetic/incoming', complete: false,
  rows: [1, 2].map((number) => ({ port_id: `leaf-${number}`, ordinal: null, target_name: `A.leaf-${number}.mov`,
...
export const batchPreview: HandoffBatchPreviewEnvelope = { ...batchObservation, batch_id: 'batch_1234567890_1234567890', expires_in_seconds: 300,
  candidates: [1, 2].map((number) => ({ candidate_handle: `candidate_1234567890_123456789${number}`, name: `A.leaf-${number}.mov`, size: 1000, action: 'copy' })),
  matches: [1, 2].map((number) => ({ port_id: `leaf-${number}`, candidate_handle: `candidate_1234567890_123456789${number}`, state: 'matched' })) }
