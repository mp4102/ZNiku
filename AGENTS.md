...
- 2026-09-14 用户已采纳源准备合同精简并授权实施，新增下位决策见
  `docs/v0.3.4-working-source-policy.md`：新建默认流程区分可读取、工作兼容与严格保内容审计；
  普通工作源使用独立 exact `0.3.4-work.1`，旧 `0.3.4`/`0.3.4-color.1` 原义不变。
  普通路线不得伪造完整审计事实，不再将 T1 晋级作为其唯一入口；T1 本身仍保持禁用及原晋级条件。
  不修改真实媒体、旧工程或用户服务，不自动选择颜色/改速/裁音频。只在声明范围内实现及验证。

- `v0.3.4` 用户已批准 `docs/v0.3.4-execution-plan.md`，连续实施 Phase 0–4，完成后可 commit/push。
  源视频检查与准备属于普通专用节点；原件只读，reference/effective/audio 显式绑定，新 exact definitions
  不改旧合同。T1 策略须通过真实代表及留出门禁才晋级；T3、priming 转换未晋级保持关闭。
  Phase 5 真实外部全链路、PR/merge/tag/Release、替换受测包或操作用户服务不包含在本次授权中。
  2026-09-14 用户已批准 `docs/v0.3.4-color-declaration-policy.md` 第 6 节；新政策使用
  exact/wire `0.3.4-color.1`，产品仍为 `0.3.4`，保留旧 `0.3.4` 定义。默认不推断源色彩，
  仅显式用户工作解释可填补已证明未指定项；不得覆盖冲突/解析失败或以此替代真实 T1 晋级。
