...
Phase 5 本机启动包已构建、验证并交付，真实全流程等待操作者执行；构建来源及交付结果见
[v0.3.6 启动包与验收说明](v0.3.6-desktop-acceptance.md)。这不是公开 Windows Release。
整个目录解压后双击 `ZNIKU Studio.exe`，无需 Python、npm 或另开后端。
本版使用独立 `Studio-v0.3.6-candidate` 本机状态，不接管旧版本；旧最近工程不自动迁移不是丢失工程。

测试新优化时，**先新建独立工程**；在创建/分析前展开“工作流版本与旧工程兼容”，选择
“0.3.6 融合编码候选（待验收）”。默认仍为 0.3.5，不能只升级 exe 就认为旧图已经切换。
“同时导出裁后章节（额外占用空间与时间）”默认不勾选，才能验证不生成整套裁后章的优化。
增强仍按章批量交回；FI 处理 `fi-input`，交回未经自行裁边的 `fi-raw`；新节点选有效帧并连续编码，
不是跳过裁边逻辑。所有外部原件及已登记成果保留，文件出现不自动提交。

暂不切换默认、不标记 Phase 5 操作者验收完成；真实性能和画面/音画同步须在本包上确认。
如要更新同一 0.3.6 候选，先在其页面“退出应用”；0.3.5 旧实例独立保留，不要从旧标签判断新包效果。
以下 v0.3.5 说明保留为原发布与操作边界；本版入口与工作流选择以上文和新版验收说明为准。
...
- 新建工程默认使用“ZNIKU 标准视频流程 · 0.3.5”。普通视频直接分析；分析进度来自工具，可取消后从头重跑。
  不支持时，可在分析页选择外部修复候选并明确确认，以候选自身帧数、帧率和音轨重新分析。
  此动作只改 Source 引用，不复制、更名或覆盖原文件；请保留候选供后续处理与归档。
- 外部源故障修复与可选第0项马赛克修复不是同一动作：后者仍须遵守参考视频 N→N 和帧序约定。
- “工作流版本与旧工程兼容”折叠区保留历史流程；已创建的新分析工程固定其 exact 版本，不能借切换选项升级工程。
...
- 意外结束应用时，Windows 只清理本实例自动处理进程树；再次打开工程继续采用 Core 的
  ...
  不属于此清理树，不会因退出被误杀。
- 本候选模式偏好与最近 8 个工程保存于 `%LOCALAPPDATA%/ZNIKU/Studio-v0.3.5-candidate`，不因随机端口变化丢失；它们不是
