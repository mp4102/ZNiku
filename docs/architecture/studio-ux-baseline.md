...
v0.3.6 的 UX-003 复用同一正式任务投影：常驻摘要、节点细进度与右侧助手不得串用历史 Run。
点击立即反馈请求等待，服务端确认后才显示受理；有测量才显示确定进度，写入 100% 不代表检查/登记完成。
可选的纯文本 `ProgressSample.stage` 仅描述当前阶段，不是新 Runtime 状态，不参与 Graph/Result/reuse。
状态摘要独占布局行，不遮挡画布；断线保留最后可信事实，不自动重放未知结果的副作用。
交回后的本机文件收纳可以展示既有 job 的真实字节进度，但不是外部 AI 完成百分比，也不将节点改为 running。
无服务起时只标本机请求等待时间；自动运行的已用时采用已有服务起时投影，不另建计时/轮询 authority。
...
2026-09-16 用户批准章级批量交付补充，具体见
[`chapter-batch-enhancement-plan.md`](../chapter-batch-enhancement-plan.md)。上述单文件导入合同继续约束
旧接口；新增独立批量入口可接收同一人工节点的多个命名输出，不是跨任务猜测匹配。用户一次选择目录或
多个文件，Python 返回逐文件匹配预览，歧义由用户指定。确认可先保存部分来件；这些仅为未提交收件，
不登记 Artifact、不宣称媒体合同通过。全部齐全后一次完整检查及显式整章 Submit，复用现有 Runtime 语义。
默认提供章级文件清单与“收到 X / N”，不伪装外部工具完成进度；同一 NodeInstance 的共享参数仍经 Schema
编辑。新批量接口可以显示和打开 attempt 内的专属批量收件目录，只观察有界单层文件，不提供任意文件管理。
原文件保留、覆盖确认、迟到绑定/文件变化拒绝和正式 Submit 重验规则不变。旧工程不自动换成新模板。
...
2026-09-16 经操作者批准的英文目录补充见
[`english-readable-storage-plan.md`](../english-readable-storage-plan.md)。磁盘采用稳定英文任务名、可选章节
和 `round-NNN`，GUI 继续使用中文任务语言。单输出与规范命名唯一的章级批量交回区平铺；只有输出重名
才使用服务端分配的可读子目录。正常路径不要求理解内部节点/Run 编号或端口哈希。助手仍绑定本次正式
handoff，不能按显示名猜目录；旧等待路径不改写，别名不触发搬家，不为界面建立第二份媒体或自动清理历史。
...
2026-09-13 经操作者批准的成片设置修订：新向导默认以 `.zniku` 所在目录作为成片父目录，并默认按
“片名（年份）”创建子文件夹，例如 `SHKD-744 (2017)`；不要求再次选择目录或事先创建片名文件夹。
工程工作数据使用专用磁盘时，不改变成片父目录的默认来源。修改成片父目录、恢复工程目录、关闭片名子目录
和允许覆盖均属于“其他选项（可选）”；默认禁止覆盖。已有 Graph 与旧调用的直接输出布局不迁移、不改写，
底层 `PublicationRequest.layout` 省略值仍为 `direct`，新向导显式提交 `title_subdirectory` 意图。

设置步骤常驻片名、年份，以及按处理顺序分组的“章节与分段”“画质增强”“章节补帧”“成片编码”。有有效
默认值可以直接使用；重要的处理决策不以“高级”名义隐藏。精确切分条件字段随方式展开，必填缺项阻止
进入分析并定位输入框；可选区存在错误时显式检查自动展开并定位。可选模型版本留空不阻止下一步。

设置步骤显示 Python 返回的输出目录与只读检查结果，确认步骤显示最终完整成品路径。默认父目录由 Python
解析工程路径，前端不拼接目录；只读自动预览不抢焦点、不启动分析，下一步始终重新检查。目录仅在开始处理
后的输出步骤按需创建；选择、预览、创建工作流和取消均不创建外部输出目录。命名和受控创建规则由 AV27
模板与通用媒体节点合同定义，不能在 TS 复制。
