...
Phase 5 [Windows 本机候选已交付](docs/v0.3.6-desktop-acceptance.md)，真实全流程等待用户验收。
本机状态使用独立 `Studio-v0.3.6-candidate`，
...
[v0.3.6 自动化验证记录](docs/v0.3.6-validation-record.md)。
...
UX-001–003 暂缓；不自动迁移旧工程，不把一次通过扩展为全部 4K、外部模型或首次用户体验保证。

## 外部马赛克修复交回（已实现，已纳入本次真实全流程）

- 输入报告提供中文摘要及格式化 JSON；视频与分析报告分开显示。
- 新建的默认 MR 任务按实际输入 stem 生成 `.RM.mp4/.mov/.mkv`，实际封装由媒体内容识别。
- 将任意名视频放入 `incoming` 后刷新，或选择原位置文件；选择不会复制。
- 点击“检查并导入”通过后才收纳，最后“提交并继续”；外部原件保留，收件区内只改名不重复复制。
- 旧已等待任务保留原文件名、容器和交回方式，不自动迁移。实现与验收说明见
  [外部马赛克修复交回体验增量](docs/mr-handoff-usability-plan.md)。

## 英文可读工程目录（已实现，已纳入本次真实全流程）
...
- 基点实现版本 `0.3.5`，原分支 `codex/v0.3.5`；从 `v0.3.3@cb7f295` 独立建立，后续在 `codex/chapter-batch-enhancement` 集成发布；保留 v0.3.4，不迁入其修复体系。
- [执行备忘录](docs/v0.3.5-execution-memo.md)先行；[源准入合同](docs/v0.3.5-source-admission-contract.md)记录实际 AV2.7 对齐范围。
- 新建工程默认标准视频流程：单次完整 packet 分析、可取消进度、显式外部修复候选重新分析。
  不自动修复或覆盖原片；候选成为新参考，不宣称它与原片逐帧相同。
- 图执行、分章分叶、可选第0步、逐叶增强、章节合并、重叠 FI 和最终输出保持同一 Graph；新精确节点不改旧工程。
- 本机状态通道 `ZNIKU/Studio-v0.3.5-candidate` 为兼容而保留；历史候选结果见备忘录，当前验收与源码 Release 边界见本版发布记录。
...
- 本机状态通道为`ZNIKU/Studio-v0.3.3-candidate`，不会接管仍在测试的v0.3.2实例。
- 开发与自动化门禁已完成，独立Windows候选可供新工程验收；实际结果、入口及限制见执行方案，不代表正式Release。

## v0.3.2 分章与重叠 FI（保留的上一开发线）

- [可读工程数据与归档定位专项](docs/v0.3.2-readable-storage-plan.md)已实现并通过自动化门禁：新工程默认
  章节/任务/批次可读目录；旧工程可显式复制整理、生成文件目录和重新定位数据副本。独立候选及操作说明见专项。
  不自动整理已验收工程，不改既有媒体结果或受测包，不据此关闭Phase 5剩余真实门禁。

- 保留的工作树分支：`v0.3.2`；起点为 v0.3.1 修复提交 `d867344`，独立目录，不切换原验收checkout。
...
- 该开发线实现版本：`0.3.2`，旧wire/Presentation仍为`0.3.0`；该线预览独立wire为`0.3.2`。
...
v0.3.5 已完成本机真实流程验收。操作者保留已验收的独立桌面包，入口见
[本机使用与验收说明](docs/v0.3.5-acceptance.md)；公开下载与已知限制见[发布记录](docs/v0.3.5-release.md)。
不要用旧工程试做格式或节点升级；旧候选、真实工程与媒体保留不动。
