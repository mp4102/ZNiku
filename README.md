...
## v0.3.4 素材检查与准备（当前开发线）

- 当前实现版本：`0.3.4`；目标版本：`ZNIKU Studio v0.3.4`，独立分支 `codex/v0.3.4`。
- 已批准[执行计划](docs/v0.3.4-execution-plan.md) Phase 0–4；当前增量及退出边界见
  [普通工作源实现记录](docs/v0.3.4-working-source-implementation.md)，旧 T1 历史结果单独保留。
- 已批准[普通工作源政策](docs/v0.3.4-working-source-policy.md)：新建默认使用 `0.3.4-work.1`。
  `source_preparation/work_*` 实现只读原件、合并检查、显式 FFV1 工作副本与新外部参考；
  `prepared_source/work_*` 复用既有章叶/FI/Final 数学和媒体实现，不另造 Core 或第二套工作流。
- 页面区分“可直接处理／需要准备／暂不支持”。缺色彩声明须明确选择工作解释，不能覆盖已知冲突或 HDR。
  重新定时须确认节奏/时长影响；外部新参考按实际新帧数与其音频重新规划，不宣称保内容修复。
- 正常路径不复制原件；一次全片帧检查聚合 N/时间/几何/工具色彩观察，准入只复核绑定和轻量可读性。
  音轨尾差不再单独导致视频拒绝，不自动裁剪或伸缩；Final 不默认进行全 PCM 恒等审计。
- 旧 `0.3.4` 与[色彩政策](docs/v0.3.4-color-declaration-policy.md)的 `0.3.4-color.1` exact/wire 原义不变。
  严格保内容路线保留为显式专用选择，T1 仍未晋级；旧工程不迁移，不复用新合同冒充旧验证。
- 当前工作源候选状态使用 `ZNIKU/Studio-v0.3.4-work-candidate`，不接管旧实例、任务或媒体。
- 本轮只验证声明范围和短合成链；AAC priming/padding 转换、HDR、真实外部 AI 与 Phase 5 不冒称完成。

## v0.3.3 原片规划与外部前处理（保留的上一开发线）

- 该线实现版本：`0.3.3`；目标版本：`ZNIKU Studio v0.3.3`，不是旧工程的自动升级声明。
