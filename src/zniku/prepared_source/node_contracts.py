"""工作源绑定 0.3.4 节点的局部来源合同与执行前检查，不引入全局图语义或读取隐藏索引。
...
OVERLAP_NAMESPACE = "zniku.source.prepared"
OVERLAP_NODE_VERSION = "0.3.4"
_ARTIFACT_ID = TypeAdapter(RandomId)
ATOMIC_SPLIT_TYPE_PREFIX = "zniku.prepared.overlap.split.leaves."
ROLE_TYPES = {
    "enhancement": "zniku.prepared.overlap.enhancement.external",
    "merge": "zniku.prepared.overlap.merge_video",
    "context": "zniku.prepared.overlap.fi_context",
    "fi": "zniku.prepared.overlap.frame_interpolation.external",
    "crop": "zniku.prepared.overlap.fi_crop",
    "program": "zniku.prepared.overlap.program_encode",
    "final": "zniku.prepared.overlap.final_mux",
}
...
def fail(code: str, message: str) -> Never:
    ...
    raise Av27MediaError(f"E_PREPARED_SOURCE_{code}", message)


class SourceExpectation(ChapterModel):
    """只绑定已有准入及工作参考；原件、参考、音频不得合并为一个来源角色。"""

    original_media_artifact_id: RandomId
    reference_media_artifact_id: RandomId
    reference_video_artifact_id: RandomId
    admission_artifact_id: RandomId
    diagnosis_artifact_id: RandomId
    audio_source_artifact_ids: Annotated[tuple[RandomId, ...], Field(min_length=1, max_length=16)]
    frame_count: PositiveInt
    frame_rate: ExactSeconds

    @field_validator("audio_source_artifact_ids", mode="before")
    @classmethod
    def audio_array(cls, value: Any) -> Any:
    ...
    def positive_rate(self) -> Self:
        ...
        if len(set(self.audio_source_artifact_ids)) != len(self.audio_source_artifact_ids):
            fail("AUDIO_BINDING", "音频载体不得重复")
        return self
...
class SourceBinding(SourceExpectation):
    ...
    mr_role: Literal["reference", "mr"]

    @model_validator(mode="after")
    def explicit_role(self) -> Self:
        if (self.mr_role == "reference") != (
            self.effective_video_artifact_id == self.reference_video_artifact_id
        ):
            fail("MR_ROLE", "有效视频必须显式对应 reference 或已验证 MR")
        return self

    def expectation(self) -> SourceExpectation:
        return SourceExpectation.model_validate(
            self.model_dump(exclude={"effective_video_artifact_id", "mr_role"})
        )
...
class OverlapMetadata(ChapterModel):
    ...
    producer_version: Literal["0.3.4"] = "0.3.4"
    role: Role
...
class SplitParameters(ChapterModel):
    ...
    def original_plan(self) -> Self:
        if (
            self.plan.source.artifact_id != self.source.reference_video_artifact_id
            or self.plan.source.frame_count != self.source.frame_count
            or self.plan.source.frame_rate != self.source.frame_rate
        ):
            fail("PLAN_SOURCE", "分章计划必须绑定已准入参考，不得绑定原件或未来 MR")
        return self
...
def read_metadata(item: RunnerInput) -> OverlapMetadata:
    ...
    try:
        ...
        raise Av27MediaError("E_PREPARED_SOURCE_METADATA", "缺少或非法的新节点 metadata") from exc
    ...
    for ordinal, item in enumerate(values):
        try:
            ...
            raise Av27MediaError(
                "E_PREPARED_SOURCE_INPUT_ID", "直接输入缺少 canonical Artifact UUID"
            ) from exc
        if (
            item.kind != kind
...
def _check_admission(gate: RunnerInput, source: SourceExpectation) -> None:
    """直接 gate 的严格合同与规划种子一致；不访问隐藏 Artifact 索引。"""
    from zniku.source_preparation.contracts import read_gate

    value = read_gate(gate)
    if (
        gate.artifact_id != source.admission_artifact_id
        or value.original_media_artifact_id != source.original_media_artifact_id
        or value.reference_media_artifact_id != source.reference_media_artifact_id
        or value.diagnosis_artifact_id != source.diagnosis_artifact_id
        or value.source_frame_count != source.frame_count
        or value.frame_rate != source.frame_rate
        or tuple(item.artifact_id for item in value.audio_bindings)
        != source.audio_source_artifact_ids
    ):
        fail("ADMISSION_BINDING", "准入与 original/reference/audio/diagnosis 身份或坐标不匹配")
...
def preflight(
    ...
    *,
    _kernel: ContractKernel | None = None,
) -> NodeContract:
    """在媒体 I/O 前失败关闭；adapter 和完成 validator 都调用，不信任 producer 自报绑定。"""
    kernel = _kernel or ContractKernel(
        read_metadata, effective_contract, _check_admission, _legacy_reference_check, _metadata
    )
    ...
    try:
        ...
        raise Av27MediaError("E_PREPARED_SOURCE_PARAMETERS", "节点参数不满足严格合同") from exc
    expected_ports = {
        "split": {"videos", "gate"},
        "merge": {"videos"},
        "context": {"chapters"},
    ...
    if isinstance(params, SplitParameters):
        ...
        if (
            ...
            fail("SOURCE_BINDING", "Split 必须绑定一个原片或工作源绑定修复输入及原片准入")
        video = videos[0]
        kernel.check_admission(gate, params.source)
        old = kernel.effective_contract(video, params.source)
        if video.artifact_id == params.source.reference_video_artifact_id:
            kernel.check_reference(video, gate)
        source = SourceBinding(
            **params.source.model_dump(),
            effective_video_artifact_id=video.artifact_id,
            mr_role="reference"
            if video.artifact_id == params.source.reference_video_artifact_id
            else "mr",
        )
    ...
    metadata = tuple(kernel.read_metadata(item) for item in items)
    _uniform(metadata)
    first = metadata[0]
    ...
    if isinstance(params, EnhancementParameters):
        ...
        if (params.mr_mode == "external") != (source.mr_role == "mr"):
            fail("FINAL_MR_MODE", "成片修复标识与实际有效输入来源不符")
        sources = _inputs(inputs, "sources", many=True, kind="MediaFile")
        gate = _inputs(inputs, "gate", many=False, kind="DataFile")[0]
        if (
            tuple(item.artifact_id for item in sources) != source.audio_source_artifact_ids
            or gate.artifact_id != source.admission_artifact_id
            or gate.producer_port_id != "gate"
        ):
            fail("FINAL_SOURCE", "Final 必须绑定准入声明的有序音频载体及同一 gate")
        kernel.check_admission(gate, source)
    result = kernel.metadata(
        cast(Role, role),
        source,
        count=count,
        geometry=geometry,
...
class ContractKernel:
    """可信 Python 包显式提供局部模型边界，共享部分只计算帧区间，不变换输入身份。

    新版本提供自己的严格 reader/factory；不得通过替换全局、伪造旧 namespace 或旧 producer
    来调用此计算过程。该对象不属于 Schema、Graph 参数或可序列化插件配置。
    """

    read_metadata: Callable[[RunnerInput], Any]
    effective_contract: Callable[[RunnerInput, SourceExpectation], legacy._MediaContract]
    check_admission: Callable[[RunnerInput, SourceExpectation], None]
    check_reference: Callable[[RunnerInput, RunnerInput], object]
    metadata: Callable[..., Any]


def _legacy_reference_check(video: RunnerInput, gate: RunnerInput) -> object:
    from zniku.source_preparation.contracts import check_reference_binding

    return check_reference_binding(video, gate)


type DeclaredContainer = Literal["mp4", "mov", "mkv"]
EXTERNAL_TYPE_PREFIX = "zniku.prepared_source.external."
...
def effective_contract(item: RunnerInput, source: SourceExpectation) -> legacy._MediaContract:
    ...
    if item.artifact_id == source.reference_video_artifact_id:
        ...
        namespace = _plain(item.media_info.get(OVERLAP_NAMESPACE))
        if (
            not isinstance(namespace, dict)
            or set(namespace) != {"role", "admission"}
            or namespace["role"] != "reference"
        ):
            fail("SOURCE_STAGE", "参考输入必须为新 SourceAdmission video 输出")
        gate = SourceGate.model_validate_json(json.dumps(namespace["admission"]))
        if (
            ...
            or tuple(item.artifact_id for item in gate.audio_bindings)
            != source.audio_source_artifact_ids
        ):
            fail("SOURCE_BINDING", "参考媒体与准入种子不一致")
        result = legacy._MediaContract(
            source.frame_count,
            Fraction(source.frame_rate),
            (gate.geometry.width, gate.geometry.height),
            gate.signal.model_dump(),
        ...
        try:
            ...
            raise Av27MediaError("E_PREPARED_SOURCE_EXTERNAL", "不是工作源绑定修复产物") from exc
