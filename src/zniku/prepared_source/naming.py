...
def overlap_output_paths(
    ...
    *,
    media_basename: str,
    _role_checker: Callable[[NodeDefinition], str | None] = definition_role,
    ...
    if (node.type_id, node.definition_version) != (
        ...
        return ()
    role = _role_checker(definition)
    if role is None or role in {"program", "final"}:
        return ()
    if role == "external":
        try:
            external = ExternalParameters.model_validate(node.model_dump(mode="json")["parameters"])
        except (ValidationError, Av27MediaError, ValueError):
            return ()
        basename = validate_media_basename(media_basename)
        return (OutputPathSpec("video", f"{basename}.RM.{external.declared_container}"),)
    try:
