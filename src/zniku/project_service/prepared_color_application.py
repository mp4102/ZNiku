...
from zniku.prepared_color.template import PreparedSourceBinding, build_prepared_source
...
from zniku.source_color.contracts import check_reference_binding, read_diagnosis
from zniku.source_color.definitions import (
    definition_role,
    source_preparation_definitions,
)
from zniku.source_color.models import DiagnosticReport, summary_for_report
from zniku.source_color.template import build_preparation_graph
...
def _interpretation(roles: Mapping[str, NodeInstance]) -> InterpretationPolicy:
    """只显示当前 Graph 中显式保存的选择；诊断结果不能自动采用用户解释。"""
    admission = roles.get("admission")
    return cast(
        InterpretationPolicy,
        admission.parameters.get("interpretation_policy", "declared_only")
...
def _parse[M: BaseModel](model: type[M], payload: object) -> M:
    try:
        ...
        raise ColorPreparedSourceError(
            "E_PREPARED_SOURCE_REQUEST_INVALID", first["msg"], field_path=first["loc"]
        ) from error


def _error(code: str, message: str) -> ColorPreparedSourceError:
    return ColorPreparedSourceError(f"E_PREPARED_SOURCE_{code}", message, http_status=409)
...
def _roles(
    snapshot: ProjectSnapshot, role_reader: Callable[..., str | None] = definition_role
...
def _check_graph(snapshot: ProjectSnapshot) -> dict[str, NodeInstance]:
    ...
    expected = build_preparation_graph(
        ...
        interpretation_policy=_interpretation(roles),
        route=_route(roles),
...
def _bound(
    ...
    request: Any,
    graph_checker: Callable[[ProjectSnapshot], dict[str, NodeInstance]] = _check_graph,
...
def _stage_progress(
...
) -> ColorPreparedSourceStageProgress | None:
    if attempt is None:
        return None
    ...
    for line in reversed(text.splitlines()):
        if line.startswith("ZNIKU_SOURCE_PROGRESS "):
            try:
                return ColorPreparedSourceStageProgress.model_validate_json(
...
def create(
    ...
    payload: object,
    *,
    request_model: Any = ColorPreparedSourceCreateRequest,
    definitions_factory: Callable[..., Any] = source_preparation_definitions,
    graph_factory: Callable[..., Graph] = build_preparation_graph,
) -> StatusEnvelope:
    """排他发布新准备工程；既有文件、已打开旧工程和原件不被覆盖。"""
    request = _parse(request_model, payload)
    with app._state:
        app._assert_idle()
        ...
        graph = graph_factory(str(source))
...
) -> tuple[ColorPreparedSourceAction, ...]:
    ...
    return (
        ColorPreparedSourceAction(
            ...
            reason=(
                ...
                if rate_choice_only
                else "原件无需内容修复；须先明确本工程工作色彩解释，再执行独立准入。"
                if summary["can_interpret"] and summary["direct_eligible_with_interpretation"]
        ...
        ColorPreparedSourceAction(
            ...
            strategy_id="t1-clock-quantization/2",
...
def view(app: ProjectServiceApplication, payload: object) -> ColorPreparedSourceViewEnvelope:
    """不扫描媒体、不创建节点；状态只由当前的正式 Run 和准入结果派生。"""
    request = _parse(ColorPreparedSourceViewRequest, payload)
    with app._state:
        ...
        if failed is not None or run.state is RunState.FAILED:
            ...
            if admission is None:
                ...
                admitted = check_reference_binding(
                    _direct(binding.reference_video, "video"),
                    _direct(binding.admission_artifact, "gate"),
                )
                working_signal_basis = (
                    "按用户在本工程中显式确认的 SDR BT.709/有限范围/left 工作解释处理缺失项；"
                    "不是原片实测声明，不是色彩转换。"
                    if any(item.source == "operator_confirmation" for item in admitted.basis)
                    else "工作解释来自完整读取的明确声明或有语法依据的规范默认值。"
                )
        ...
        return ColorPreparedSourceViewEnvelope(
            ...
            interpretation_policy=_interpretation(roles),
            color_interpretation_available=bool(summary and summary["can_interpret"]),
            color_interpretation_required=bool(summary and summary["missing_color_fields"]),
            color_interpretation_reason=(
                "等待完整色彩声明检查。"
                if summary is None
                else "文件未完整注明所需色彩；只有确认适合 SDR BT.709 时，才主动采用工作解释。"
                if summary["can_interpret"]
                else "已有冲突、解析失败或未支持语义，不能通过工作解释选项覆盖；请查看检查发现。"
                if summary["missing_color_fields"]
                else "不需要用用户假设填补色彩；仍须通过其余素材与音频检查。"
            ),
            working_signal_basis=working_signal_basis,
            handoff=None
            if waiting is None
...
def cancel(app: ProjectServiceApplication, payload: object) -> StatusEnvelope:
    ...
    from .prepared_color_operations import operation_cancel
...
def choose(app: ProjectServiceApplication, payload: object) -> StatusEnvelope:
    ...
    request = _parse(ColorPreparedSourceChooseRequest, payload)
    with app._state:
        app._assert_idle()
        if app._desktop_closing:
            raise _error("CLOSING", "应用正在关闭。")
        ...
        summary = summary_for_report(report)
        # 解释选择是修改 Admission 参数的用户决定，不是对诊断事实的修订。
        # 未完成的解析和明确冲突不能因传了 enum 而成为已知工作信号。
        if request.interpretation_policy != "declared_only" and not summary["can_interpret"]:
            raise _error(
                "COLOR_INTERPRETATION_UNAVAILABLE",
                "当前结果不能采用缺失色彩解释；明确冲突、解析失败及未支持语义必须先解决。",
            )
        if (
            request.route == "direct"
            and summary["missing_color_fields"]
            and request.interpretation_policy == "declared_only"
        ):
            raise _error(
                "COLOR_INTERPRETATION_REQUIRED",
                "素材缺少必要声明。默认保持阻断；如有依据，请显式选择本工程工作色彩解释。",
            )
...
def _completed_binding(
    ...
    roles: Mapping[str, NodeInstance],
    attempts: Mapping[str, NodeRun],
    reference_checker: Callable[[RunnerInput, RunnerInput], Any] = check_reference_binding,
    ...
    gate = reference_checker(_direct(reference_video, "video"), _direct(gate_artifact, "gate"))
...
def processing_preview(payload: object) -> ColorPreparedSourceProcessingEnvelope:
    request = _parse(ColorPreparedSourceProcessingRequest, payload)
    return ColorPreparedSourceProcessingEnvelope(processing=request.processing)


def full(
    app: ProjectServiceApplication, payload: object, *, expand: bool
) -> ColorPreparedSourceFullEnvelope | StatusEnvelope:
    """在同一锁内重验 CAS/直接输入并预览或保存普通完整图，保存不自动运行。"""
    request = _parse(ColorPreparedSourceFullRequest, payload)
    with app._state:
        app._assert_idle()
        if app._desktop_closing:
            raise _error("CLOSING", "应用正在关闭。")
        ...
        if expand:
            ...
            app._last_error = None
            return app.inspect()
        return ColorPreparedSourceFullEnvelope(
            project_session_id=request.project_session_id,
...
def dispatch(app: ProjectServiceApplication, action: str, payload: object) -> BaseModel:
    ...
    # 同一路由以显式 exact 分流。受信实现由本机代码注册，客户端不能注入 helper。
    if isinstance(payload, Mapping) and payload.get("contract_version") == "0.3.4-work.1":
        from .work_application import dispatch as work_dispatch

        return work_dispatch(app, action, payload)
    ...
    try:
        ...
        if action in {"operation-view", "operation-cancel"}:
            from .prepared_color_operations import operation_cancel, operation_view
    ...
    except ColorPreparedSourceError:
        raise
        ...
        raise ColorPreparedSourceError(error.code, str(error)) from error
    except (
        ProjectStoreError,
        RuntimeRepositoryError,
        ValidationError,
        ...
        raise ColorPreparedSourceError("E_PREPARED_SOURCE_OPERATION_FAILED", str(error)) from error
