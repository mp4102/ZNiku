...
from zniku.avenhance_v27 import MOSAIC_RESTORATION_TYPE_ID
...
from zniku.source_aligned.timeline import require_supported_audio_origins
...
def parse_source_aligned[M: BaseModel](model: type[M], payload: object) -> M:
    ...
    try:
        ...
        location = tuple(
            ...
            if part not in {"average", "exact_frames", "exact_times", "external", "off"}
        )
        raise SourceAlignedError(
            "E_SOURCE_ALIGNED_REQUEST_INVALID", first["msg"], field_path=location
        ) from error


def processing_preview(payload: object) -> SourceAlignedProcessingEnvelope:
    """分析前只验证处理合同，不启动节点或猜测N/FPS。"""

    request = parse_source_aligned(SourceAlignedProcessingRequest, payload)
    return SourceAlignedProcessingEnvelope(processing=request.processing)


def full_source_aligned(
    app: ProjectServiceApplication,
    payload: object,
    *,
    expand: bool,
) -> SourceAlignedFullEnvelope | StatusEnvelope:
    """同锁重验并展开；仅save成功后返回新status，不自动启动Run。"""

    from .service import ProjectServiceError

    request = parse_source_aligned(SourceAlignedFullRequest, payload)
    with app._state:
        try:
            ...
            if any(
                node.type_id == MOSAIC_RESTORATION_TYPE_ID
                for node in current.snapshot.project.graph.nodes
            ):
                raise ProjectServiceError(
                    "E_SOURCE_ALIGNED_LEGACY_PREPARATION",
                    "此工程包含旧外部修复节点。请沿用旧工作流，或新建仅原片分析的工程；不会自动迁移旧结果。",
                    http_status=409,
                )
            preparation, binding = app._resolve_av27_preparation_run(
                request.preparation_run_id, overlap=True
            )
            if binding.mr_mode != "off":
                raise ProjectServiceError(
                    "E_SOURCE_ALIGNED_LEGACY_PREPARATION",
                    "此分析包含旧外部修复节点。请沿用旧工作流，或新建仅原片分析的工程；不会自动迁移旧结果。",
                    http_status=409,
                )
            if current.snapshot.project.graph != preparation.project.graph:
                raise ProjectServiceError(
                    "E_SOURCE_ALIGNED_GRAPH_ALREADY_EXPANDED",
                    "当前图已展开或含自定义节点。请在节点图编辑，不会由向导覆盖现有流程。",
                    http_status=409,
                )
            if binding.source_mode != "program" or len(binding.sources) != 1:
                raise ProjectServiceError(
                    "E_SOURCE_ALIGNED_SOURCE_MODE",
                    "新路线只支持单一原片时间轴；已分章素材请沿用旧工作流。",
                    http_status=409,
                )
            for source in binding.sources:
                for artifact in (
                    ...
                    try:
                        ...
                        raise ProjectServiceError(
                            "E_SOURCE_ALIGNED_INPUT_CHANGED",
                            "分析输入已不可用，请重新分析",
                            http_status=409,
                        ) from error
                    if (
            ...
            # 在昂贵增强/FI开始前短探测原音轨首包与首解码样本；不静默转码或丢弃预滚语义。
            # 此处与运行节点复用同一媒体检查，预览仍不登记结果或改变原片。
            require_supported_audio_origins(Path(binding.sources[0].source_media_artifact.path))
            build = build_source_aligned(
                preparation, binding, request.processing, request.publication
            )
            if expand:
                store.save(
                    build.project,
                ...
                return app.inspect()
            return SourceAlignedFullEnvelope(
            ...
            raise SourceAlignedError(error.code, str(error), field_path=("source",)) from error
        except GraphValidationError as error:
            raise SourceAlignedError("E_SOURCE_ALIGNED_GRAPH_INVALID", str(error)) from error
