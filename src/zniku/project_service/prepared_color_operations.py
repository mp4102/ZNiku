...
def _attempt(
    run: Run, node_run_id: str, role_reader: Callable[..., str | None] = definition_role
) -> tuple[NodeRun, str]:
    """只接受 snapshot 中完整 exact 准备定义的最新 attempt，不从 UI 别名推断。"""
    from .prepared_color_application import _error
...
def _run(app: ProjectServiceApplication, request: Any) -> Run:
...
def _active_attempt(
    ...
    run: Run,
    role_reader: Callable[..., str | None] = definition_role,
) -> NodeRun | None:
    """manual 用显式临时绑定，automatic 用当前唯一 RUNNING 受信节点；不猜 waiting 次序。"""
    from .prepared_color import ColorPreparedSourceError
    ...
    if manual_target is not None:
        try:
            attempt, role = _attempt(run, manual_target, role_reader)
    ...
    if len(running) != 1:
        return None
    try:
        attempt, role = _attempt(run, running[0].node_run_id, role_reader)
    except ColorPreparedSourceError:
        return None
...
def operation_view(
    ...
    from .prepared_color_application import _parse, _stage_progress

    request = _parse(ColorPreparedSourceOperationRequest, payload)
    with app._state:
        ...
        return ColorPreparedSourceOperationEnvelope(
            **request.model_dump(),
            active=active,
...
def operation_cancel(
    ...
    from .prepared_color_application import _error, _parse

    request = (
        _parse(ColorPreparedSourceViewRequest, payload)
        if run_only
        else _parse(ColorPreparedSourceOperationRequest, payload)
    )
    with app._state:
        run = _run(app, request)
        active = _active_attempt(app, run)
        if isinstance(request, ColorPreparedSourceOperationRequest):
            _attempt(run, request.node_run_id)
        if active is None or (
            isinstance(request, ColorPreparedSourceOperationRequest)
