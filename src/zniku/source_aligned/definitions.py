"""声明独立的 0.3.3 重叠 FI 节点，旧 AV27 定义与 exact 行为完全不变。
...
def _cached_definition(role: str, count: int) -> NodeDefinition:
    ...
    executor = (
        ...
        if manual
            ...
            adapter=f"zniku.source_aligned.adapters:{_NAMES[role]}", output_paths=paths
        )
    )
    return NodeDefinition(
        ...
        executor=executor,
        validator=ValidatorSpec(adapter=f"zniku.source_aligned.validators:validate_{_NAMES[role]}"),
    )
...
def overlap_python_adapters() -> Mapping[str, PythonAdapter]:
    ...
    module = import_module("zniku.source_aligned.adapters")
    ...
    for role, name in _NAMES.items():
        ...
        result[f"zniku.source_aligned.adapters:{name}"] = cast(PythonAdapter, value)
    return result


def overlap_validators() -> Mapping[str, NodeValidator]:
    module = import_module("zniku.source_aligned.validators")
    ...
    for name in _NAMES.values():
        ...
        result[f"zniku.source_aligned.validators:validate_{name}"] = cast(NodeValidator, value)
    result["zniku.source_aligned.validators:validate_external"] = cast(
...
def external_type_id(container: DeclaredContainer = "mp4") -> str:
    if container not in {"mp4", "mov", "mkv"}:
        raise ValueError("E_SOURCE_ALIGNED_CONTAINER: 不支持的容器")
...
def _cached_external_definition(container: DeclaredContainer) -> NodeDefinition:
    ...
    return NodeDefinition(
        ...
        executor=ManualExternalExecutorSpec(
            ...
            instructions="处理完整原片并保持 N→N、帧序、精确帧率及几何/色彩；不得裁剪、变速、"
            "补丢帧或补帧。提交实际声明容器的文件。时间轴检查不证明逐帧内容或模型。",
        ),
        validator=ValidatorSpec(adapter="zniku.source_aligned.validators:validate_external"),
    )
