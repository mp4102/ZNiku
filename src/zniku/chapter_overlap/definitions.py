"""声明独立的 0.3.2 重叠 FI 节点，旧 AV27 定义与 exact 行为完全不变。
...
def _cached_definition(role: str, count: int) -> NodeDefinition:
    ...
    executor = (
        ...
        if manual
            ...
            adapter=f"zniku.chapter_overlap.adapters:{_NAMES[role]}", output_paths=paths
        )
    )
    return NodeDefinition(
        ...
        validator=ValidatorSpec(
            adapter=f"zniku.chapter_overlap.validators:validate_{_NAMES[role]}"
        ),
    )
...
def overlap_python_adapters() -> Mapping[str, PythonAdapter]:
    ...
    module = import_module("zniku.chapter_overlap.adapters")
    ...
    for role, name in _NAMES.items():
        ...
        result[f"zniku.chapter_overlap.adapters:{name}"] = cast(PythonAdapter, value)
    return result


def overlap_validators() -> Mapping[str, NodeValidator]:
    module = import_module("zniku.chapter_overlap.validators")
    ...
    for name in _NAMES.values():
        ...
        result[f"zniku.chapter_overlap.validators:validate_{name}"] = cast(NodeValidator, value)
    return result
