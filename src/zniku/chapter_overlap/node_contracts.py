"""重叠 FI 节点的局部来源合同与执行前检查，不引入全局图语义或读取隐藏索引。
...
from .context import _context_geometry
from .models import ChapterLeafPlan, ChapterModel, ExactSeconds, Ordinal, PositiveInt

OVERLAP_NAMESPACE = "zniku.chapter.overlap"
OVERLAP_NODE_VERSION = "0.3.2"
...
def fail(code: str, message: str) -> Never:
    ...
    raise Av27MediaError(f"E_OVERLAP_{code}", message)


class SourceBinding(ChapterModel):
    """唯一有效视频及原音轨、准入 Artifact 绑定，不以路径或人类标题代替身份。"""
...
def validate_chapter(source: SourceBinding, chapter: ChapterBinding) -> None:
...
class OverlapMetadata(ChapterModel):
    ...
    producer_version: Literal["0.3.2"] = "0.3.2"
    role: Role
    source: SourceBinding
...
def read_metadata(item: RunnerInput) -> OverlapMetadata:
    ...
    try:
        ...
        raise Av27MediaError("E_OVERLAP_METADATA", "缺少或非法的新节点 metadata") from exc
    ...
    for ordinal, item in enumerate(values):
        try:
            ...
            raise Av27MediaError(
                "E_OVERLAP_INPUT_ID", "直接输入缺少 canonical Artifact UUID"
            ) from exc
        if (
            item.kind != kind
...
def _check_admission(gate: RunnerInput, source: SourceBinding) -> None:
    ...
    try:
        ...
        raise Av27MediaError("E_OVERLAP_ADMISSION", "无法读取严格准入 JSON") from exc
    if (
...
def preflight(
    ...
    try:
        ...
        raise Av27MediaError("E_OVERLAP_PARAMETERS", "节点参数不满足严格合同") from exc
    expected_ports = {
        "split": {"videos", "gate"},
        "merge": {"videos"},
        "context": {"chapters"},
    ...
    if isinstance(params, SplitParameters):
        ...
        if (
            ...
            or gate.artifact_id != params.admission_artifact_id
        ):
            fail("SOURCE_BINDING", "新 Split 只接受一个有效视频及绑定的准入 gate")
        video = videos[0]
        old = legacy._metadata_contract(video)
        namespace = namespace_from_media_info(video.media_info)
        stage = namespace.get("stage")
        if not (
            (type(namespace.get("source_ordinal")) is int and namespace.get("source_ordinal") == 0)
            or (isinstance(stage, Mapping) and stage.get("kind") == "mosaic_restoration")
        ):
            fail("SOURCE_STAGE", "Split 只接受已准入 Source/MR，不接受旧 Split/FI")
        if (
            video.producer_port_id != "video"
            or video.artifact_id != params.plan.source.artifact_id
            or (
                old.frame_count != params.plan.source.frame_count
                or old.frame_rate != Fraction(params.plan.source.frame_rate)
            )
        ):
            fail("SOURCE_BINDING", "有效视频 Artifact/N/FPS 与规划不一致")
        source = SourceBinding(
            effective_video_artifact_id=video.artifact_id,
            admission_artifact_id=gate.artifact_id,
            source_media_artifact_id=params.source_media_artifact_id,
            frame_count=old.frame_count,
            frame_rate=params.plan.source.frame_rate,
        )
    ...
    if any(item.source != source for item in metadata):
