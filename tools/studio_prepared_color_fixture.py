...
def generate_sources(root: Path) -> tuple[Path, Path, Path]:
    ...
    # 两条 copy 路线具有相同容器声明。不能直接复制首次编码文件，因为第二次 FFmpeg
    # 封装会显式写入 left 色度位置；首次编码容器没有该项，反向交付应被新政策正确拒绝。
...
def main() -> None:
    ...
    with TemporaryDirectory(prefix=".test-tmp-color-studio-", dir=repository) as temporary:
        root = Path(temporary)
