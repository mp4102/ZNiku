...
from zniku.source_preparation import definitions
...
def test_real_direct_check_admission_preview_expand_reopen(media: Path, tmp_path: Path) -> None:
    ...
    assert isinstance(preview, PreparedSourceFullEnvelope)
...
def test_layout_is_not_execution_identity_but_source_change_is(media: Path, tmp_path: Path) -> None:
    ...
    assert isinstance(moved_view, PreparedSourceViewEnvelope)
    assert moved_view.state == "needs_choice"
    media.write_bytes(media.read_bytes() + b"changed")
    with pytest.raises(PreparedSourceError, match="INPUT_CHANGED"):
...
def test_http_wire_unknown_field_and_cancel_idle_fail_closed(media: Path, tmp_path: Path) -> None:
    ...
    with _serve(app) as (url, _server):
        status, payload, _ = _request(
            url, PREPARED_SOURCE_PREFIX + "/view", method="POST", payload=request
        )
        assert status == 200 and payload["contract_version"] == "0.3.4"
        Draft202012Validator(PreparedSourceViewEnvelope.model_json_schema()).validate(payload)
        status, payload, _ = _request(
            url,
            PREPARED_SOURCE_PREFIX + "/choose",
            method="POST",
            payload={
                **request,
                "expected_storage_revision": app.inspect().storage_revision,
        ...
        status, payload, _ = _request(
            url, PREPARED_SOURCE_PREFIX + "/cancel", method="POST", payload=request
        )
...
def test_cancel_worker_fails_without_artifact_and_releases_busy(
    ...
    assert isinstance(failure, PreparedSourceViewEnvelope)
