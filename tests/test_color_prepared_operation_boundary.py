...
from zniku.source_color import build_preparation_graph, source_preparation_definitions
    ...
    *,
    ...
    def make_attempt(node_id: str, identity: str) -> NodeRun:
        return NodeRun(
            ...
            run_id=run_id,
            node_id=node_id,
            definition_version="0.3.4-color.1",
            attempt=1,
...
def test_operation_wire_rejects_unknown_fields_without_signalling() -> None:
    case = _operation()
    with pytest.raises(ColorPreparedSourceError, match="REQUEST_INVALID"):
        dispatch(case.app, "operation-cancel", {**case.request, "force": True})
    assert not case.signal.is_set()


def test_old_operation_endpoint_cannot_reinterpret_new_exact_snapshot() -> None:
    """改变请求版本不改变实际 snapshot 的定义归属，不能借旧端点发停止信号。"""
    ...
    with pytest.raises(PreparedSourceError):
        old_dispatch(case.app, "operation-cancel", {**case.request, "contract_version": "0.3.4"})
    assert not case.signal.is_set()


def test_new_operation_endpoint_cannot_reinterpret_old_exact_snapshot() -> None:
    from test_prepared_source_operation_boundary import _operation as old_operation
