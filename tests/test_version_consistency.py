"""锁定 0.3.2 开发产品身份，并拒绝把产品版本扩散到旧 wire、节点定义或工程格式。"""
...
def test_development_product_identity_is_0_3_2() -> None:
    pyproject: dict[str, Any] = tomllib.loads((ROOT / "pyproject.toml").read_text("utf-8"))

    assert (ROOT / "VERSION").read_text("utf-8").strip() == "0.3.2"
    assert pyproject["project"]["version"] == "0.3.2"
    assert zniku.__version__ == "0.3.2"
    ...
    assert studio_package["version"] == "0.3.2"
    assert studio_lock["version"] == "0.3.2"
    assert studio_lock["packages"][""]["version"] == "0.3.2"
    assert [package["version"] for package in zniku_packages] == ["0.3.2"]
