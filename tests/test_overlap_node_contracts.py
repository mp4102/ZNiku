...
from zniku.runtime import FrameRange, NodeExecutionRequest, NodeValidatorContext, RunnerInput
...
def pipeline(
    ...
    *,
    ...
    source_data = steps[0].contract.source.model_dump(mode="json")
    ...
    add("final", (*program, original, gate), {"source": source_data, "mr_mode": "off"})
    return steps
...
def test_admission_rejects_same_length_unrelated_original_source(tmp_path: Path) -> None:
    ...
    wrong = dict(step.parameters, source_media_artifact_id=str(uuid4()))
...
def test_all_definitions_strict_roundtrip_and_no_old_identity_replacement(tmp_path: Path) -> None:
    ...
    assert len(definitions) == 8 and len(overlap_validators()) == 8
