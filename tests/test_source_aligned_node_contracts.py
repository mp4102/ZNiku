...
from zniku.source_aligned import validators
...
def synthetic_audio_origins(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(validators, "verify_video_span", lambda path, **kwargs: None)
...
def pipeline(
    ...
    *,
    ...
    namespace = {
        ...
        "duration_seconds": float(n / Fraction(rate)),
        "source_ordinal": 0,
        "audio_tracks": [],
    }
    source = RunnerInput(
    ...
    gate = RunnerInput("gate", gate_id, "DataFile", gate_path, producer_port_id="gate")
    steps: list[Step] = []
    ...
    expectation = SourceExpectation(
        original_video_artifact_id=source_id,
        source_media_artifact_id=audio_id,
    ...
    if restored:
        external = ExternalMetadata(
            producer_type_id="zniku.source_aligned.external.mp4",
            source=expectation,
...
def test_full_local_pipeline_all_eight_validators_probe_and_extend_only_new_namespace(
    ...
    for step in steps:
        ...
        assert probed == [o.path for o in context.outputs]
        assert all(
...
def test_direct_input_wrong_source_and_old_namespace_fail_before_probe(
    ...
    for mode in ("source", "old", "producer"):
        ...
        if mode == "old":
            ...
            if mode == "source":
                data[OVERLAP_NAMESPACE]["source"]["original_video_artifact_id"] = str(uuid4())
            else:
...
def test_admission_rejects_same_length_unrelated_original_source(tmp_path: Path) -> None:
    ...
    wrong = dict(
        step.parameters,
        source=dict(step.parameters["source"], source_media_artifact_id=str(uuid4())),
    )
    ...
    with pytest.raises(Av27MediaError, match="ADMISSION"):
...
def test_effective_binding_appears_only_at_runtime_and_propagates(
    ...
    assert (source.effective_video_artifact_id != source.original_video_artifact_id) == restored
    for step in steps:
    ...
    assert steps[0].parameters["plan"]["source"]["artifact_id"] == source.original_video_artifact_id
...
def test_new_exact_definitions_do_not_change_old_definitions() -> None:
    ...
    assert [item.type_id for item in current] == [item.type_id for item in previous]
    assert all(item.version == "0.3.2" for item in previous)
    assert all(item.version == "0.3.3" for item in current)
    assert previous == old()
